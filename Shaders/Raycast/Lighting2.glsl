...
Intersect RenderScene(vec3 lightPos, vec3 orig, vec3 direction)
{
	...
	for(float x = -3.0; x <= 3.0; x += 0.2)
	{
		...
		float newT = sphere(spherePos, orig, direction);
			...
			{	
				...
				intersection.color = vec3(intersection.M.x,intersection.M.y,intersection.M.x);
			}
		y += 0.2;
	}
	return intersection;
...
Intersect ReflectRay(vec3 orig, vec3 dest, vec3 normal)
{
	...
	vec3 reflectedRay = normalize(reflect(dest-orig, normal));
	Intersect intersection = RenderScene(lightPos, orig, reflectedRay);
	return intersection;
...
void main()
{
	...
	Intersect intersection = RenderScene(lightPos, camP, camDir);
	...
	vec3 colorLighting = intersection.color;
	...
	//Color calculation
	vec3 color = abs(normalize(colorLighting+reflectedColor + vec3(1.9, 0.0, 0.0)));
