...
void main()
{
	...
	for(int i = 0; i <= maxIterations; i++)
	{
		...
		if(newT < glowEpsilon) glowT += glowT - glowEpsilon;
		t = newT;
		...
		if(newT <= epsilon) 
		{
			...
			gl_FragColor = mix(color*glowT*0.00001, vec4(0.8,0.5,1.0,1.0), length(newPos-camP)/200);
			return;
		}
		...
		else gl_FragColor = mix(sphereColor*glowT*0.00001, vec4(0.8,0.5,1.0,1.0), length(newPos-camP)/100);
